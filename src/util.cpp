...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No smartcash.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override smartcash.conf
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure SmartCash Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("SmartCash Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The SmartCash Core developers";
    }
