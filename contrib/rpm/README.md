...
The RPM spec file provided here is for SmartCash-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://smartcash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[SmartCash git repository](https://github.com/smartcash/smartcash) but are not part of
...
Currently only `Source10` is used, for the example `smartcash.conf` file.
...
in the smartcash git repository. At present only an SVG version of the SmartCash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the smartcash
...
needed when 0.12.1 is released, a proper fix is already in the SmartCash git
master branch.
...
idea to build SmartCash Core against a static version of that library compiled
...
The `smartcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `smartcash-server` package. At this point, most RPM based
...
The `smartcash-server` package should have SELinux support. How to properly do
...
SmartCash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/smartcash/). He has been
packaging SmartCash for Fedora at least since 2012.
...
4. I renamed the `smartcash` package that contains the Qt GUI to `smartcash-core` as
that appears to be how the general population refers to it, in contrast to
`smartcash-xt` or `smartcash-classic`. I wanted to make sure the general population
