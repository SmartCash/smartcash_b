#!/bin/bash
# Copyright (c) 2013 The SmartCash Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete SmartCash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no SmartCash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old SmartCash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected SmartCash 0.7 datadir."
    ;;
  3)
    echo "Detected SmartCash pre-0.8 datadir."
    ;;
  4)
    echo "Detected SmartCash 0.8 datadir."
    ;;
esac
