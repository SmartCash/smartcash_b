...
*Setup instructions for a Gitian build of SmartCash Core using a Debian VM or physical system.*
...
- [Building SmartCash Core](#building-smartcash-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-smartcash-host/README.md).
...
Clone the git repositories for smartcash and Gitian.
...
in the smartcash repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building SmartCash Core
----------------

To build SmartCash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the smartcash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/smartcash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/smartcash/smartcash
...
For example:
```bash
URL=https://github.com/laanwj/smartcash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit smartcash=${COMMIT} --url smartcash=${URL} ../smartcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit smartcash=${COMMIT} --url smartcash=${URL} ../smartcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit smartcash=${COMMIT} --url smartcash=${URL} ../smartcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the smartcash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../smartcash/contrib/gitian-descriptors/*|sort|uniq )
...
SMARTCASHPATH=/some/root/path/smartcash
SIGPATH=/some/root/path/smartcash-detached-sigs

./bin/gbuild --url smartcash=${SMARTCASHPATH},signature=${SIGPATH} ../smartcash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/smartcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/smartcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/smartcash-osx-build.assert
```
...
[smartcash-core/gitian.sigs](https://github.com/smartcash-core/gitian.sigs/) repository, or if that's not possible create a pull
