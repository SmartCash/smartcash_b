SmartCash Core version 0.11.0 is now available from:

  <https://smartcash.org/bin/smartcash-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer SMARTCASH/kB fee to be
...
For example, add the following to `smartcash.conf`:
...
related to the blockchain in the smartcash system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows SmartCash Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that SmartCash Core will 
...
For an example script see [smartcash-submittx](https://github.com/laanwj/smartcash-submittx).
...
- #5334 `cf87536` libsmartcashconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new smartcash-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only SmartCashAmountField
- #5753 `0f44672` Add smartcash logo to about screen
...
- #5809 `46bfbe7` Add smartcash-cli man page
...
- #5849 `41259ca` contrib/init/smartcashd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename SmartCash-Qt.app to SmartCash-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for smartcash-tx)
